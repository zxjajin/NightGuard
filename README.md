...
- 夜间探索型工具默认 23:00 - 23:30 只提醒，23:30 - 07:00 进入收束提醒
...
- 最小化到系统托盘
- Codex / Cursor 等夜间探索型工具收束提醒
- 夜间收束提醒支持“本晚不再限制”
- 选择“记录到明天并最小化”会记录最近动作，并尝试最小化对应应用主窗口
- 选择“稍后再提醒我”会在 15 分钟后重新提醒
- 本地 JSON 配置和日志
...
- `NightFocusProcesses`：夜间探索型工具进程列表，默认包含 `Codex.exe`、`Cursor.exe`
- `NightFocusDomains`：夜间探索型网站 hosts 限制列表

## 夜间探索型工具收束

NightGuard 对 Codex / Cursor 等探索型工具使用单独的夜间节奏：

- `23:00 - 23:30`：只弹出一次收尾提醒，不拦截
- `23:30 - 07:00`：发现目标应用主窗口时弹出收束提醒
- “本晚不再限制”：本晚剩余时间不再提醒该应用
- “记录到明天并最小化”：写入最近动作记录，并尝试最小化对应应用主窗口
- “稍后再提醒我”：暂缓 15 分钟，之后重新提醒
